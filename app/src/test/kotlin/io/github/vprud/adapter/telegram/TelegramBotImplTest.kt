package io.github.vprud.adapter.telegram

import com.github.kotlintelegrambot.errors.TelegramError
import io.github.vprud.service.SubscriptionService
import io.github.vprud.task.RecurrentTaskExecutor
import io.mockk.*
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.ExperimentalCoroutinesApi
import kotlinx.coroutines.test.UnconfinedTestDispatcher
import kotlinx.coroutines.test.resetMain
import kotlinx.coroutines.test.setMain
import org.junit.jupiter.api.AfterEach
import org.junit.jupiter.api.BeforeEach
import org.junit.jupiter.api.assertDoesNotThrow
import kotlin.test.AfterTest
import kotlin.test.BeforeTest
import kotlin.test.Test

@OptIn(ExperimentalCoroutinesApi::class)
class TelegramBotImplTest {
    private val testDispatcher = UnconfinedTestDispatcher()
    private lateinit var commandHandlers: Map<String, CommandHandler>
    private lateinit var errorHandler: ErrorHandler
    private lateinit var recurrentTaskExecutor: RecurrentTaskExecutor
    private lateinit var subscriptionService: SubscriptionService
    private lateinit var botService: TelegramBotImpl

    @BeforeTest
    fun setup() {
        Dispatchers.setMain(testDispatcher)
    }

    @AfterTest
    fun tearDown() {
        Dispatchers.resetMain()
        unmockkAll()
    }

    @BeforeEach
    fun setupEach() {
        commandHandlers =
            mapOf(
                "start" to mockk(),
                "help" to mockk(),
                "subscribe" to mockk(),
                "unsubscribe" to mockk(),
                "mysubscriptions" to mockk(),
            )
        errorHandler = mockk()
        recurrentTaskExecutor = mockk()
        subscriptionService = mockk()
        botService = TelegramBotImpl(commandHandlers, errorHandler, recurrentTaskExecutor)
    }

    @AfterEach
    fun tearDownEach() {
        clearAllMocks()
    }

    @Test
    fun `startBot should initialize bot and start polling`() {
        val token = "test-token"
        coEvery { recurrentTaskExecutor.start(any()) } just Runs

        botService.startBot(token, subscriptionService)

        coVerify { recurrentTaskExecutor.start(any()) }
    }

    @Test
    fun `stopBot should stop polling and issue checking`() {
        val token = "test-token"
        coEvery { recurrentTaskExecutor.start(any()) } just Runs
        coEvery { recurrentTaskExecutor.stop() } just Runs
        botService.startBot(token, subscriptionService)

        botService.stopBot()

        coVerify { recurrentTaskExecutor.stop() }
    }

    @Test
    fun `should handle error messages correctly`() {
        val token = "test-token"
        val error = mockk<TelegramError>()
        coEvery { errorHandler.handleError(error) } just Runs
        coEvery { recurrentTaskExecutor.start(any()) } just Runs

        botService.startBot(token, subscriptionService)

        assertDoesNotThrow {
            errorHandler.handleError(error)
        }
    }
}
